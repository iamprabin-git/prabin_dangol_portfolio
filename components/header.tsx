...
export function Header({ site }: { site: SiteContent }) {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 flex flex-col border-b border-[var(--line)] transition-colors",
        open ? "h-dvh bg-[var(--bg)]" : "",
        !open && scrolled
    ...
    >
      ...
      <div className="relative z-50 shrink-0 bg-[var(--bg)]">
        ...
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-2 px-4 sm:px-8">
          <Link href="/" className="group flex min-w-0 flex-1 items-center gap-2.5 sm:gap-3" onClick={() => setOpen(false)}>
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
            ...
            <button
              type="button"
              className="relative z-50 grid h-10 w-10 place-items-center rounded-full border border-[var(--line)] bg-[var(--bg)] lg:hidden"
              aria-expanded={open}
      ...
      {open ? (
        <div
          id="mobile-nav"
          className="relative z-40 min-h-0 flex-1 overflow-y-auto bg-[var(--bg)] pb-[max(1.5rem,env(safe-area-inset-bottom))] lg:hidden"
        >
